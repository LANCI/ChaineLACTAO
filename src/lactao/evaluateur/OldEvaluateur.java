...
/* fonctionne avec l'ancienne structure de la table classe */
public class OldEvaluateur {
	...
	public ArrayList<ArrayList<Double>> getMatrixForClass(int classeNo) throws Exception {
		...
		ResultSet rsDomifs = cnn.getRS("SELECT no_segment FROM classe WHERE no_classe =" + classeNo);
		while (rsDomifs.next()) {

			String sql = "";
			sql += " SELECT unif, freq FROM matrice";
			sql += " WHERE no_domif = " + rsDomifs.getInt("no_segment");
			sql += "   AND NOT exclue";
	...
	public double getGlobalMSE(String dbFileName) throws Exception {
		...
		initDic();
		double mse = 0;
		ResultSet rs = cnn.getRS("SELECT DISTINCT no_classe FROM classe");
		int count = 0;
		while (rs.next()) {
	...
	public static void main(String args[]) {
		try {
			System.out.println((new OldEvaluateur()).getGlobalMSE("c:\\bergson\\bergson_som_evolution.mdb"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
